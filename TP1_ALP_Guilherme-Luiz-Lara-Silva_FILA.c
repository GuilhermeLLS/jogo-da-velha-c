...
void FilaVazia(Jogada Fila[9]){
  	Fila[0].Head = 0;
  	Fila[0].Tail = Fila[0].Head;
}  /* FLVazia */

void Enfileirar(int i, Jogada Fila[i]){
  Fila[i].Tail = Fila[i++].Tail;
}
void Desenfileirar(int i, Jogada Fila[i]){
	Fila[i].Head = Fila[i].Head;
}
//--------------------------------------
//FUNÇÃO QUE PESQUISA JOGADA
/*void PesquisaJogada(Jogada Fila[9], int numeroJogadas){
    int Jog_Esc, z, i;
    ...
    printf("A jogada escolhida foi : &d",Fila[z]);
}
...
Jogada escolheJogada(int (*tabuleiro)[3][3], int jogador){
  ...
  // Escolhe a primeira jogada disponível entre as jogadas disponíveis
...
void escreveJogadas(HANDLE* arq, char* nome_arq, int jogador, int numeroJogadas, Jogada jogadas[9]){
  ...
  for(i = 0; i < (numeroJogadas - 1); i++){    // percorre o vetor de jogadas já realizadas
    ...
    saida[10 + i*5] = '\n';                   // quebra de linha - parte 2
  	Enfileirar(i, jogadas);
  	Desenfileirar(i,jogadas);
  }
